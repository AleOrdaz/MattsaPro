...
final EvidenceFormProviderAfter = StateNotifierProvider.autoDispose
...
class EvidenceFormNotifier extends StateNotifier<EvidenceFormState> {
  ...
  void adapativeConditions() async {
    final alllist = await liftingRepository.getAllLiftAfter(lifting.id);
    final p = 0;
  ...
  Future<bool> enviarimagen(
    ...
    final res = await liftingRepository.uploadImageafter(
        image, idLifting, idClass, comentario);
    final newevidences = await liftingRepository.getAllLiftAfter(idLifting);
